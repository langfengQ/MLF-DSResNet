from parallel_nets.spike_layer_for_dvscifar10 import *
...
class ResNet(nn.Module):
    """ Establish ResNet.
     ...
     """
    def __init__(self, block, num_block_layers, num_classes):
        super(ResNet, self).__init__()

        self.conv1 = nn.Conv2d(2, 32, 3, 1, 1, bias=False)
        self.bn1 = tdBatchNorm(nn.BatchNorm2d(32))
        self.layer1 = BLock_Layer(block, 32, 32, num_block_layers[0], False, modified=True)
        self.layer2 = BLock_Layer(block, 32, 64, num_block_layers[1], True, modified=True)
        self.layer3 = BLock_Layer(block, 64, 128, num_block_layers[2], True, modified=True)
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.voting = nn.Linear(128, num_classes)
        self.spike_func = MLF_unit()
...
def resnet14():
    return ResNet(BasicBlock, [2, 2, 2], 10)
